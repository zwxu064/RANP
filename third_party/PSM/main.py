...
def train(imgL, imgR, disp_L):
   ...
   #---------
    ...
    if args.model == 'stackhourglass':
        ...
        loss = 0.5 * F.smooth_l1_loss(output1[mask], disp_true[mask], reduction='elementwise_mean') \
               + 0.7 * F.smooth_l1_loss(output2[mask], disp_true[mask], reduction='elementwise_mean') \
               + F.smooth_l1_loss(output3[mask], disp_true[mask], reduction='elementwise_mean')
    elif args.model == 'basic':
        output = model(imgL,imgR)
        output = torch.squeeze(output,1)
        loss = F.smooth_l1_loss(output[mask], disp_true[mask], reduction='elementwise_mean')
