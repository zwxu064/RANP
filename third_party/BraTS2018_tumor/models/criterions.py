...
def hard_cross_entropy(output, target, alpha=3.0, ignore_index=255):
...
def hard_per_im_cross_entropy(output, target, alpha=3.0, ignore_index=255):
    n, c = output.shape[:2]
...
def focal_loss(output, target, alpha=0.25, gamma=2.0, ignore_index=255):
    n = target.size(0)
...
def mean_cross_entropy(output, target, alpha=3.0, ignore_index=255):
    mtx = F.cross_entropy(output, target, reduce=False, ignore_index=ignore_index)
...
def cross_entropy_dice(output, target, weight=1.0, ignore_index=255):
...
def cross_entropy_dice_per_im(output, target, weight=1.0, ignore_index=255):
    loss = weight * F.cross_entropy(output, target, ignore_index=ignore_index)
    output = F.softmax(output, dim=1)
    for c in range(1, 5):
        o = output[:, c]
