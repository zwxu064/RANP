...
import numpy as np
sys.path.append('third_party/miccai/Dataloader')
sys.path.append('../../..')

from torch.utils.data import Dataset
from nilearn.image import resample_to_img
from data_augmentation import *
from aux.utils import RandomCrop3D
...
if __name__ == '__main__':
  ...
  # Validation
  validset = BraTSDataset('train', '../../../datasets/BraTS', [2018], spatial_size=128,
                          enable_random_offset=False,
                          enable_random_crop=False,
                          enable_random_flip=False)
  enable_save_mat = True

  if enable_save_mat:
    import scipy.io as scio
    save_dir = '/home/users/u5710355/Desktop/visual/BraTS_18'

  for idx in range(validset.__len__()):
    obj = validset.__getitem__(idx)
    ...
    gt = np.transpose(gt, [0, 3, 2, 1])
    if len(gt.unique()) == 4:
      multi_slice_viewer('manual', [data[0], data[1], data[2], data[3], gt[0]], ['t1', 't1ce', 't2', 'flair', 'seg'])
      plt.show()

      if enable_save_mat:
        scio.savemat('{}/brats_2018_train_{}.mat'.format(save_dir, idx),
                     {'t1': data[0].permute(2, 1, 0).numpy(),
                      't1ce': data[1].permute(2, 1, 0).numpy(),
                      't2': data[2].permute(2, 1, 0).numpy(),
                      'flair': data[3].permute(2, 1, 0).numpy(),
                      'seg': gt[0].permute(2, 1, 0).numpy()})