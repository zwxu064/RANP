...
def zero_ops(m, x, y):
    m.total_ops += torch.Tensor([int(0)]).double()
    m.output_size = torch.Tensor(list(y.size())).byte()
...
def count_convNd(m, x, y):
    ...
    m.total_ops += torch.Tensor([int(total_ops)]).double()
    m.output_size = torch.Tensor(list(y.size())).byte()
...
def count_convNd_ver2(m, x, y):
    ...
    m.total_ops += torch.Tensor([int(output_size * kernel_ops)]).double()
    m.output_size = torch.Tensor(list(y.size())).byte()
...
def count_relu(m, x, y):
    ...
    m.total_ops += torch.Tensor([int(nelements)]).double()
    m.output_size = torch.Tensor(list(y.size())).byte()
...
def count_softmax(m, x, y):
    ...
    batch_size, nfeatures = x.size()[0:2]
...
def count_upsample(m, x, y):
    ...
    if m.mode == "linear":
        ...
        total_ops = y.nelement().double() * 11 # 6 muls + 5 adds
    elif m.mode == "bicubic":
