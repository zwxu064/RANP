  ...
  for key in obj.keys():
    if tb_mode == 'scalar':
      if key in {'train_loss', 'valid_loss', 'acc', 'acc_acl',
                 ...
                 'valid_acc', 'train_whole', 'train_enhance', 'train_core',
                 'valid_whole', 'valid_enhance', 'valid_core', 'train_loss_iter',
                 'train_iou', 'valid_iou'}:
...
class RandomCrop3D(object):
  def __init__(self, size=None, enable_crop_random_seed=False):
    if isinstance(size, numbers.Number):
  ...
  def __call__(self, inputs):
    ...
    if self.enable_crop_random_seed:
      h_rand, w_rand, d_rand = random.randint(0, max(0, h_gap-1)), random.randint(0, max(0, w_gap-1)), \
                               random.randint(0, max(0, d_gap-1))
    else:
      h_rand, w_rand, d_rand = h_gap // 2, w_gap // 2, d_gap // 2

    for input in inputs:
      if input is not None:
        h_range_right = -(h_gap - h_rand) if h_gap > 0  else h
        w_range_right = -(w_gap - w_rand) if w_gap > 0 else w
        d_range_right = -(d_gap - d_rand) if d_gap > 0 else d

        new_input = input[:, h_rand:h_range_right, w_rand:w_range_right, d_rand:d_range_right]
...
def model_transfer_last_layer(model, in_channel, target_class, weight_init='xn'):
  # First layer for different number of input in_channel
  found_first_conv = False
  for idx, (key, layer) in enumerate(model.encoders.named_modules()):
    if (not found_first_conv) and isinstance(layer, nn.Conv3d):
      found_first_conv = True
      if layer.weight.size(1) != in_channel:
        out_c = layer.weight.size(0)
        layer.weight = nn.Parameter(layer.weight.new_zeros(out_c, in_channel, 3, 3, 3), requires_grad=True)
        layer.in_channels = in_channel
        weight_init_single_layer(layer, weight_init, enable_bias=False)

  # Last layer for different number of classes
  if model.final_conv.weight.size(0) != target_class:
    in_c = model.final_conv.weight.size(1)
    model.final_conv.weight = nn.Parameter(model.final_conv.weight.new_zeros(target_class, in_c, 1, 1, 1), requires_grad=True)
    model.final_conv.out_channels = target_class

    if model.final_conv.bias is not None:
      model.final_conv.bias = nn.Parameter(model.final_conv.bias.new_zeros(target_class), requires_grad=True)

    weight_init_single_layer(model.final_conv, weight_init, enable_bias=True)

  return model


def weight_init_single_layer(layer, weight_init, enable_bias):
  if weight_init == 'ort':
    nn.init.orthogonal_(layer.weight)
  elif weight_init == 'fixed':
